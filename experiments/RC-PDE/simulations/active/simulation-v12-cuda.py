...
Quick run commands (copy/paste):
  # Best full-v12 run (interactive / non-headless)
  python simulations/active/simulation-v12-cuda.py --identity-cap-fraction 0.25 --coherence-bg-floor 0.02 --identity-birth-gate-fraction 0.9

  # Best full-v12 run (headless, reproducible artifacts)
  python simulations/active/simulation-v12-cuda.py --headless --headless-steps 5000 --nx 1024 --ny 1024 --dx 0.1 --seed 1 --storage-mode disk --snapshot-interval 50 --identity-cap-fraction 0.25 --coherence-bg-floor 0.02 --identity-birth-gate-fraction 0.9

  # Standard interactive run (VisPy by default)
  python simulations/active/simulation-v12-cuda.py

  # Matplotlib-only run
  python simulations/active/simulation-v12-cuda.py --no-vispy

  # Headless run (no live UI; snapshots + offline animation)
  python simulations/active/simulation-v12-cuda.py --headless --storage-mode disk --snapshot-interval 50

  # Long headless run with disk snapshots (recommended for 1024x1024)
  python simulations/active/simulation-v12-cuda.py --headless --storage-mode disk --snapshot-interval 100 --nx 1024 --ny 1024

  # Memory snapshots (short runs)
  python simulations/active/simulation-v12-cuda.py --headless --storage-mode memory --snapshot-interval 20

Help:
  python simulations/active/simulation-v12-cuda.py --help
...
def _parse_args():
    parser = argparse.ArgumentParser(
        description="RC-PDE v12 CUDA simulation",
...
I_global_mass_cap      = None    # set from initial total coherence mass after initialization
...
def update_identities(C, I_tensor, n_active, spark_mask, g_xx, g_xy, g_yy, dt, step):
    ...
    if n_active > 0:
        ...
        total_mass = torch.sum(I_tensor[:n_active]) * dx * dy
        ...
        masses_post = torch.sum(I_tensor[:n_active], dim=(1, 2)) * dx * dy
...
# Identity mass cap follows total coherence mass budget.
I_global_mass_cap = target_mass0_value
