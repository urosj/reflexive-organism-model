"""
RC-PDE v14 CUDA: GPU-accelerated RC-v12/13 branch with soft-closure controls.

What v14 is trying to achieve:
  - Keep the reflexive core intact (C<->g coupling + coherence-mass projection).
  - Reduce reliance on hard RC-III event gates by blending toward continuous, field-driven closures.
  - Let you sweep between hard-gated behavior and softer dynamics via CLI (closure_softness family),
    so you can probe how close behavior stays to RC motifs without dropping back to fixed-PDE artifacts.

Quick run commands (copy/paste):
  # Best full-v14 run (interactive / non-headless)
  python simulations/active/simulation-v14-cuda.py --closure-softness 0.8 --spark-softness 0.08 --collapse-softness 0.5 --identity-cap-fraction 0.25 --coherence-bg-floor 0.02 --identity-birth-gate-fraction 0.9

  # Best full-v14 run (headless, reproducible artifacts)
  python simulations/active/simulation-v14-cuda.py --headless --headless-steps 5000 --nx 1024 --ny 1024 --dx 0.1 --seed 1 --storage-mode disk --snapshot-interval 50 --closure-softness 0.8 --spark-softness 0.08 --collapse-softness 0.5 --identity-cap-fraction 0.25 --coherence-bg-floor 0.02 --identity-birth-gate-fraction 0.9

  # Interactive run (VisPy if available)
  python simulations/active/simulation-v14-cuda.py

  # Force matplotlib UI
  python simulations/active/simulation-v14-cuda.py --no-vispy

  # Headless long run with disk snapshots (recommended)
  python simulations/active/simulation-v14-cuda.py --headless --headless-steps 5000 --storage-mode disk --snapshot-interval 50

  # Large grid run
  python simulations/active/simulation-v14-cuda.py --headless --storage-mode disk --snapshot-interval 100 --nx 1024 --ny 1024

  # Hard RC-III-style gating (no soft closure blending)
  python simulations/active/simulation-v14-cuda.py --headless --closure-softness 0 --seed 1

  # Softer field-driven closure behavior
  python simulations/active/simulation-v14-cuda.py --headless --closure-softness 0.8 --spark-softness 0.08 --collapse-softness 0.5

  # Identity budget controls
  python simulations/active/simulation-v14-cuda.py --identity-cap-fraction 0.25 --coherence-bg-floor 0.02 --identity-birth-gate-fraction 0.9

Help:
  python simulations/active/simulation-v14-cuda.py --help
"""
...
def _parse_args():
    parser = argparse.ArgumentParser(
        description="RC-PDE v14 CUDA simulation",
        ...
        epilog=(
            ...
            "  python simulations/active/simulation-v14-cuda.py\n"
            "  python simulations/active/simulation-v14-cuda.py --no-vispy\n"
            "  python simulations/active/simulation-v14-cuda.py --headless --storage-mode disk --snapshot-interval 50\n"
            "  python simulations/active/simulation-v14-cuda.py --headless --storage-mode disk --snapshot-interval 100 --nx 1024 --ny 1024\n"
        ),
    )
...
def compute_spark_mask(C, g_xx, g_xy, g_yy, step=None):
    ...
    # Hard detector (RC-III style) blended with soft detector (continuous instability field).
    ...
    spark_mask = (1.0 - closure_softness) * spark_hard + closure_softness * spark_soft

    if step is not None and step % 50 == 0:
        spark_pixels = torch.count_nonzero(spark_mask > 0.5)
        spark_strength = torch.sum(spark_mask).item()
        print(f"[SPARK] step={step}: px={spark_pixels} strength={spark_strength:.1f}")
...
def update_identities(C, I_tensor, n_active, spark_mask, sqrt_g, dt, step):
    ...
    hard_prune_mass = id_min_mass * (1.0 - 0.5 * closure_softness)
    spark_threshold = 0.5 - 0.35 * closure_softness
    ...
    sparks_factor_t = (1.0 - closure_softness) * sparks_hard_t + closure_softness * sparks_soft_t
    ...
    mass_factor_t = (1.0 - closure_softness) * mass_hard_t + closure_softness * mass_soft_t
    ...
    slots_factor_t = (1.0 - closure_softness) * slots_hard_t + closure_softness * slots_soft_t
    ...
    if id_birth_interval <= 1:
        ...
        interval_sigma = 0.25 + 0.25 * closure_softness
        interval_soft_t = torch.exp(-(dist_t * dist_t) / (2.0 * interval_sigma * interval_sigma))
    interval_factor_t = (1.0 - closure_softness) * interval_hard_t + closure_softness * interval_soft_t
...
if not HEADLESS_MODE:
    if USE_VISPY:
        viewer = RCVispyViewer(Nx, Ny, C_min, C_max, title="RC-PDE v14 CUDA")
    else:
        fig = plt.figure(figsize=(13,7))
        gs = fig.add_gridspec(2, 3, height_ratios=[6,1])
