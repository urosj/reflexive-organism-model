"""
RC-PDE v16 CUDA: PDE-boundary scaffold with nonlocal/operator/domain hooks.

What v16 iteration-1 is trying to achieve:
  - Keep v15-equivalent default runtime behavior for baseline parity.
  - Introduce explicit v16 scaffold flags: nonlocal/operator/domain modes.
  - Preserve closure-mode ablations (`off|soft|full`) from v15.

Quick run commands (copy/paste):
  # Best full-v16 run (interactive / non-headless)
  ...
  # Best full-v16 run (headless, reproducible artifacts)
  ...
  # Telemetry-focused run (JSONL hook stream in outputs/readback-telemetry)
  RC_READBACK_HOOKS=1 python simulations/active/simulation-v16-cuda.py --headless --headless-steps 2000 --nx 512 --ny 512 --seed 1 --storage-mode memory --snapshot-interval 50 --closure-mode soft --nonlocal-mode on --operator-diagnostics --operator-diagnostics-interval 10 --domain-mode adaptive --domain-adapt-strength 0.30 --domain-adapt-interval 50

  # Telemetry + sparse field dump + explicit read-back ablation strength
  RC_READBACK_HOOKS=1 RC_READBACK_FIELD_DUMP=1 RC_READBACK_FIELD_DOWNSAMPLE=4 RC_READBACK_FIELD_INTERVAL=10 RC_READBACK_BETA_RB=1.0 python simulations/active/simulation-v16-cuda.py --headless --headless-steps 2000 --nx 512 --ny 512 --seed 1 --storage-mode memory --snapshot-interval 50 --closure-mode soft --nonlocal-mode on --operator-diagnostics --operator-diagnostics-interval 10 --domain-mode adaptive --domain-adapt-strength 0.30 --domain-adapt-interval 50

  # Interactive run (VisPy if available)
  python simulations/active/simulation-v16-cuda.py

  # Force matplotlib UI
  python simulations/active/simulation-v16-cuda.py --no-vispy

  # Headless long run with disk snapshots (recommended)
  python simulations/active/simulation-v16-cuda.py --headless --headless-steps 5000 --storage-mode disk --snapshot-interval 50
  ...
  python simulations/active/simulation-v16-cuda.py --headless --closure-mode off --seed 1

  # Core+events run (L0+L1 signal, no L2 closure identities)
  python simulations/active/simulation-v16-cuda.py --headless --closure-mode off --events-control-in-core --seed 1

  # Closure routing (default behavior: closure active)
  python simulations/active/simulation-v16-cuda.py --headless --closure-mode soft --seed 1

  # Closure softness family
  python simulations/active/simulation-v16-cuda.py --headless --closure-softness 0.8 --spark-softness 0.08 --collapse-softness 0.5

  # Identity budget controls
  python simulations/active/simulation-v16-cuda.py --identity-cap-fraction 0.25 --coherence-bg-floor 0.02 --identity-birth-gate-fraction 0.9

  # v16 scaffold flags (iteration-1 parity defaults shown)
  python simulations/active/simulation-v16-cuda.py --nonlocal-mode off --domain-mode fixed --operator-diagnostics
...
def _parse_args():
    parser = argparse.ArgumentParser(
        description="RC-PDE v16 CUDA simulation",
        ...
        epilog=(
            ...
            "  python simulations/active/simulation-v16-cuda.py --headless --closure-mode off --seed 1\n"
            "  python simulations/active/simulation-v16-cuda.py --headless --closure-mode off --events-control-in-core --seed 1\n"
            "  python simulations/active/simulation-v16-cuda.py --headless --storage-mode disk --snapshot-interval 50\n"
            "  python simulations/active/simulation-v16-cuda.py --headless --storage-mode disk --snapshot-interval 100 --nx 1024 --ny 1024\n"
            "  RC_READBACK_HOOKS=1 python simulations/active/simulation-v16-cuda.py --headless --headless-steps 2000 --nx 512 --ny 512 --seed 1 --storage-mode memory --snapshot-interval 50 --closure-mode soft --nonlocal-mode on --operator-diagnostics --operator-diagnostics-interval 10 --domain-mode adaptive --domain-adapt-strength 0.30 --domain-adapt-interval 50\n"
            "  RC_READBACK_HOOKS=1 RC_READBACK_FIELD_DUMP=1 RC_READBACK_FIELD_DOWNSAMPLE=4 RC_READBACK_FIELD_INTERVAL=10 RC_READBACK_BETA_RB=1.0 python simulations/active/simulation-v16-cuda.py --headless --headless-steps 2000 --nx 512 --ny 512 --seed 1 --storage-mode memory --snapshot-interval 50 --closure-mode soft --nonlocal-mode on --operator-diagnostics --operator-diagnostics-interval 10 --domain-mode adaptive --domain-adapt-strength 0.30 --domain-adapt-interval 50\n"
        ),
    ...
    parser.add_argument("--closure-mode", choices=["off", "soft", "full"], default="soft", help="v16 routing mode: off=L0 core-only (no events/closure identities), soft/full=closure active")
    ...
    parser.add_argument("--nonlocal-mode", choices=["off", "on"], default="off", help="v16 scaffold: enable nonlocal PDE path")
    parser.add_argument("--nonlocal-strength", type=float, default=0.05, help="v16 scaffold: nonlocal contribution strength")
    parser.add_argument("--nonlocal-kernel-passes", type=int, default=3, help="v16 scaffold: smoothing passes used by nonlocal proxy")
    parser.add_argument("--operator-diagnostics", action="store_true", help="v16 scaffold: emit operator diagnostics metadata fields")
    parser.add_argument("--operator-diagnostics-interval", type=int, default=10, help="v16 scaffold: interval for operator diagnostics collection")
    parser.add_argument("--domain-mode", choices=["fixed", "adaptive"], default="fixed", help="v16 scaffold: fixed domain or adaptive proxy mode")
    parser.add_argument("--domain-adapt-strength", type=float, default=0.0, help="v16 scaffold: adaptive-domain proxy strength (0 keeps fixed behavior)")
    parser.add_argument("--domain-adapt-interval", type=int, default=50, help="v16 scaffold: adaptive-domain proxy update interval")
...
if ARGS.nonlocal_strength < 0.0:
    raise ValueError("--nonlocal-strength must be >= 0")
if ARGS.nonlocal_kernel_passes < 1:
    raise ValueError("--nonlocal-kernel-passes must be >= 1")
if ARGS.operator_diagnostics_interval < 1:
    raise ValueError("--operator-diagnostics-interval must be >= 1")
if ARGS.domain_adapt_strength < 0.0:
    raise ValueError("--domain-adapt-strength must be >= 0")
if ARGS.domain_adapt_interval < 1:
    raise ValueError("--domain-adapt-interval must be >= 1")
...
id_cont_source_scale  = 0.15  # continuous substrate source from intrinsic spark field
id_cont_collapse_ref  = 0.08  # collapse observability reference level
...
nonlocal_strength      = ARGS.nonlocal_strength
nonlocal_kernel_passes = ARGS.nonlocal_kernel_passes
operator_diagnostics   = ARGS.operator_diagnostics
operator_diag_interval = ARGS.operator_diagnostics_interval
domain_mode            = ARGS.domain_mode
domain_adapt_strength  = ARGS.domain_adapt_strength
domain_adapt_interval  = ARGS.domain_adapt_interval
NONLOCAL_KERNEL_SIGMA_CELLS = float(nonlocal_kernel_passes)
NONLOCAL_KERNEL_FFT = None
DOMAIN_BASE_GRID_X = None
DOMAIN_BASE_GRID_Y = None
...
DETG_MAX    = 1e3
DETK_MIN    = 1e-6
OP_CONDK_OCC_THRESH = 1e3
OP_DETK_OCC_THRESH  = 10.0 * DETK_MIN
OP_DETG_OCC_THRESH  = 10.0 * DETG_MIN
DOMAIN_SCALE_GAIN = 0.15
...
def _build_nonlocal_kernel_fft(shape, dev):
    """Build FFT of a normalized periodic Gaussian kernel for nonlocal convolution."""
    nx, ny = shape
    x = torch.arange(nx, device=dev, dtype=torch.float32)
    y = torch.arange(ny, device=dev, dtype=torch.float32)
    x = torch.minimum(x, torch.tensor(float(nx), device=dev) - x)
    y = torch.minimum(y, torch.tensor(float(ny), device=dev) - y)
    r2 = (x.unsqueeze(1) * dx) ** 2 + (y.unsqueeze(0) * dy) ** 2

    sigma = max(dx, dy) * max(1.0, NONLOCAL_KERNEL_SIGMA_CELLS)
    kernel = torch.exp(-r2 / (2.0 * sigma * sigma))
    kernel = kernel / (torch.sum(kernel) + eps)
    return torch.fft.rfft2(kernel)


def compute_nonlocal_proxy(C):
    """
    v16 iteration-2 nonlocal core:
    explicit integral-like operator via periodic FFT convolution with a broad Gaussian kernel.
    """
    global NONLOCAL_KERNEL_FFT

    if nonlocal_mode != "on" or nonlocal_strength <= 0.0:
        return torch.zeros_like(C)

    fft_width = C.shape[1] // 2 + 1
    if (
        NONLOCAL_KERNEL_FFT is None
        or NONLOCAL_KERNEL_FFT.device != C.device
        or NONLOCAL_KERNEL_FFT.shape != (C.shape[0], fft_width)
    ):
        NONLOCAL_KERNEL_FFT = _build_nonlocal_kernel_fft(C.shape, C.device)

    conv = torch.fft.irfft2(torch.fft.rfft2(C) * NONLOCAL_KERNEL_FFT, s=C.shape)
    return conv - C


def compute_nonlocal_diagnostics(C):
    """Return mean-abs and RMS magnitude of the active nonlocal term."""
    if nonlocal_mode != "on" or nonlocal_strength <= 0.0:
        return 0.0, 0.0
    nonlocal_term = nonlocal_strength * compute_nonlocal_proxy(C)
    nonlocal_mean = torch.mean(torch.abs(nonlocal_term))
    nonlocal_rms = torch.sqrt(torch.mean(nonlocal_term * nonlocal_term) + eps)
    return float(nonlocal_mean.item()), float(nonlocal_rms.item())


def _zero_operator_state():
    ...
    return {
        ...
        "detg_min_t": zero,
        "g_drift_rms_t": zero,
        "degeneracy_detK_frac_t": zero,
        "degeneracy_condK_frac_t": zero,
        "degeneracy_detg_frac_t": zero,
    }


def _zero_domain_state():
    zero = torch.tensor(0.0, device=device)
    ...
    return {
        ...
        "mass_before_t": zero,
        "mass_after_t": zero,
        "mass_rel_error_t": zero,
        "mass_target_rel_error_t": zero,
        "I_mass_before_t": zero,
        "I_mass_after_t": zero,
        "I_mass_rel_error_t": zero,
    }


def _ensure_domain_base_grid(shape, dev):
    """Cache normalized sampling grid components for adaptive-domain remap."""
    global DOMAIN_BASE_GRID_X, DOMAIN_BASE_GRID_Y
    nx, ny = shape
    if (
        DOMAIN_BASE_GRID_X is not None
        and DOMAIN_BASE_GRID_Y is not None
        and DOMAIN_BASE_GRID_X.device == dev
        and DOMAIN_BASE_GRID_X.shape == (nx, ny)
    ):
        return DOMAIN_BASE_GRID_X, DOMAIN_BASE_GRID_Y

    ys = torch.linspace(-1.0, 1.0, nx, device=dev, dtype=torch.float32)
    xs = torch.linspace(-1.0, 1.0, ny, device=dev, dtype=torch.float32)
    gy, gx = torch.meshgrid(ys, xs, indexing='ij')
    DOMAIN_BASE_GRID_X = gx
    DOMAIN_BASE_GRID_Y = gy
    return DOMAIN_BASE_GRID_X, DOMAIN_BASE_GRID_Y


def remap_field_bilinear(A, scale_t):
    """Remap 2D/3D fields with a uniform zoom proxy using bilinear sampling."""
    gx, gy = _ensure_domain_base_grid((A.shape[-2], A.shape[-1]), A.device)
    grid = torch.stack((gx * scale_t, gy * scale_t), dim=-1).unsqueeze(0)
    ...
    raise ValueError(f"Unsupported tensor rank for remap_field_bilinear: {A.dim()}")
...
def update_continuous_identity(I_cont, C, spark_soft, sqrt_g, dt):
    """Evolve continuous identity substrate used in intrinsic/core paths."""
    lapI = laplacian(I_cont)
    source = id_cont_source_scale * id_birth_amp * spark_soft
    dIdt = g_id * C * I_cont - d_id * I_cont + D_id * lapI + source
    I_new = I_cont + dt * dIdt
    torch.clamp_(I_new, min=0.0)

    total_mass = torch.sum(I_new * sqrt_g) * dx * dy
    ...
    I_new *= scale
    I_mass = torch.sum(I_new * sqrt_g) * dx * dy
    return I_new, I_mass


def compute_continuous_collapse_scores(I_cont):
    """Collapse observability scores for continuous identity substrate."""
    ref = max(1e-4, id_cont_collapse_ref * id_birth_amp)
    score = torch.sigmoid((ref - I_cont) / (0.5 * ref + eps))
    return torch.mean(score), torch.max(score)


def compute_closure_observables(identity_mass_t, spark_soft, step):
    """Readout-only birth observables for closure-mode=off intrinsic runs."""
    sparks_factor_t = torch.clamp(torch.mean(spark_soft), min=0.0, max=1.0)
    birth_gate_mass_cap = I_birth_gate_fraction * I_global_mass_cap
    mass_factor_t = torch.clamp((birth_gate_mass_cap - identity_mass_t) / (birth_gate_mass_cap + eps), min=0.0, max=1.0)
    slots_factor_t = torch.tensor(1.0, device=device)

    if id_birth_interval <= 1:
        interval_factor_t = torch.tensor(1.0, device=device)
    else:
        ...
        interval_sigma = 0.30
        interval_factor_t = torch.exp(-(dist_t * dist_t) / (2.0 * interval_sigma * interval_sigma))

    birth_score_t = sparks_factor_t * mass_factor_t * slots_factor_t * interval_factor_t
    n_new_t = torch.clamp(id_birth_per_step * birth_score_t, min=0.0)
    zero = torch.tensor(0.0, device=device)
    return {
...
def update_metric_from_K(C, Jx, Jy, g_xx, g_xy, g_yy, I_sum):
    ...
    g_xx, g_xy, g_yy, det_g_after = regularise_metric(g_xx, g_xy, g_yy)
    _hook(
        "post_g_postblend",
        detg_min=torch.min(det_g_after),
        detg_mean=torch.mean(det_g_after),
        G_frob_field=torch.sqrt(g_xx * g_xx + 2.0 * g_xy * g_xy + g_yy * g_yy + eps),
    )

    if operator_diagnostics:
        traceK = K_xx + K_yy
        discK = torch.sqrt(torch.clamp((K_xx - K_yy) ** 2 + 4.0 * K_xy * K_xy, min=0.0))
        eig1 = 0.5 * (traceK + discK)
        eig2 = 0.5 * (traceK - discK)
        eig_abs1 = torch.abs(eig1)
        eig_abs2 = torch.abs(eig2)
        eig_max = torch.maximum(eig_abs1, eig_abs2)
        eig_min = torch.minimum(eig_abs1, eig_abs2)
        condK = eig_max / (eig_min + eps)
        ...
        operator_state = {
            "detK_mean_t": torch.mean(detK),
            "detK_min_t": torch.min(detK),
            "condK_mean_t": torch.mean(condK),
            "condK_max_t": torch.max(condK),
            "detg_mean_t": torch.mean(det_g_after),
            "detg_min_t": torch.min(det_g_after),
            "g_drift_rms_t": g_drift,
            "degeneracy_detK_frac_t": torch.mean((torch.abs(detK) <= OP_DETK_OCC_THRESH).to(torch.float32)),
            "degeneracy_condK_frac_t": torch.mean((condK >= OP_CONDK_OCC_THRESH).to(torch.float32)),
            "degeneracy_detg_frac_t": torch.mean((det_g_after <= OP_DETG_OCC_THRESH).to(torch.float32)),
        }
    else:
        zero = torch.tensor(0.0, device=device)
        operator_state = {
    ...
    return g_xx, g_xy, g_yy, operator_state
...
def rhs_C(C, g_xx, g_xy, g_yy, spark_mask, I_sum):
    ...
    if I_sum is not None:
        ...
        dCdt = dCdt_flux
    dCdt = dCdt + nonlocal_strength * compute_nonlocal_proxy(C)
...
def rk2_step(C, g_xx, g_xy, g_yy, target_mass, spark_mask, I_sum):
    ...
    g_xx, g_xy, g_yy, operator_state = update_metric_from_K(C_new, Jx2, Jy2, g_xx, g_xy, g_yy, I_sum)
    # Re-project after metric update so reported mass stays invariant on the final metric.
    C_new = project_to_invariant(C_new, g_xx, g_xy, g_yy, target_mass)

    return C_new, g_xx, g_xy, g_yy, dt, v2, w2, operator_state
...
print(
    f"v16 scaffold: nonlocal_mode={nonlocal_mode}, nonlocal_strength={nonlocal_strength:.3f}, "
    f"domain_mode={domain_mode}, domain_adapt_strength={domain_adapt_strength:.3f}, "
    f"operator_diagnostics={operator_diagnostics}"
)
...
class SnapshotManager:
    ...
    def __init__(self, storage_mode='memory', snapshot_dir='./snapshots'):
        ...
        self.meta = {
            ...
            'eta_id': eta_id,
            'alpha_id': alpha_id,
            'id_cont_source_scale': id_cont_source_scale,
            'id_cont_collapse_ref': id_cont_collapse_ref,
            ...
            'nonlocal_kernel_passes': nonlocal_kernel_passes,
            'domain_mode': domain_mode,
            'domain_adapt_strength': domain_adapt_strength,
            'domain_adapt_interval': domain_adapt_interval,
            'domain_scale_gain': DOMAIN_SCALE_GAIN,
            'operator_diagnostics': operator_diagnostics,
            'operator_diag_interval': operator_diag_interval,
            'operator_condk_occupancy_threshold': OP_CONDK_OCC_THRESH,
            'operator_detk_occupancy_threshold': OP_DETK_OCC_THRESH,
            'operator_detg_occupancy_threshold': OP_DETG_OCC_THRESH,
            ...
            'operator_metrics': [
                ...
                'operator_detg_mean',
                'operator_detg_min',
                'operator_g_drift_rms',
                'operator_degeneracy_detK_frac',
                'operator_degeneracy_condK_frac',
                'operator_degeneracy_detg_frac',
            ],
            'nonlocal_metrics': [
                'nonlocal_term_mean',
                'nonlocal_term_rms',
            ...
            'domain_metrics': [
                ...
                'domain_mass_before',
                'domain_mass_after',
                'domain_mass_rel_error',
                'domain_mass_target_rel_error',
                'domain_I_mass_before',
                'domain_I_mass_after',
                'domain_I_mass_rel_error',
            ],
    ...
    def add_snapshot(self, step, C, sqrt_g, spark_mask,
                    ...
                    nonlocal_term_mean_val=None, nonlocal_term_rms_val=None,
                    operator_detK_mean_val=None, operator_detK_min_val=None,
                    operator_condK_mean_val=None, operator_condK_max_val=None,
                    operator_detg_mean_val=None, operator_detg_min_val=None,
                    operator_g_drift_rms_val=None,
                    operator_degeneracy_detK_frac_val=None, operator_degeneracy_condK_frac_val=None,
                    operator_degeneracy_detg_frac_val=None,
                    domain_applied_val=None, domain_scale_val=None, domain_pressure_val=None,
                    domain_mass_before_val=None, domain_mass_after_val=None,
                    domain_mass_rel_error_val=None, domain_mass_target_rel_error_val=None,
                    domain_I_mass_before_val=None, domain_I_mass_after_val=None,
                    domain_I_mass_rel_error_val=None):
        ...
        if self.storage_mode == 'memory':
            ...
            if nonlocal_term_mean_val is not None:
                self.diagnostics.setdefault('nonlocal_term_mean', []).append(nonlocal_term_mean_val)
            if nonlocal_term_rms_val is not None:
                self.diagnostics.setdefault('nonlocal_term_rms', []).append(nonlocal_term_rms_val)
            if operator_detK_mean_val is not None:
                self.diagnostics.setdefault('operator_detK_mean', []).append(operator_detK_mean_val)
            if operator_detK_min_val is not None:
                self.diagnostics.setdefault('operator_detK_min', []).append(operator_detK_min_val)
            if operator_condK_mean_val is not None:
                self.diagnostics.setdefault('operator_condK_mean', []).append(operator_condK_mean_val)
            if operator_condK_max_val is not None:
                self.diagnostics.setdefault('operator_condK_max', []).append(operator_condK_max_val)
            if operator_detg_mean_val is not None:
                self.diagnostics.setdefault('operator_detg_mean', []).append(operator_detg_mean_val)
            if operator_detg_min_val is not None:
                self.diagnostics.setdefault('operator_detg_min', []).append(operator_detg_min_val)
            if operator_g_drift_rms_val is not None:
                self.diagnostics.setdefault('operator_g_drift_rms', []).append(operator_g_drift_rms_val)
            if operator_degeneracy_detK_frac_val is not None:
                self.diagnostics.setdefault('operator_degeneracy_detK_frac', []).append(operator_degeneracy_detK_frac_val)
            if operator_degeneracy_condK_frac_val is not None:
                self.diagnostics.setdefault('operator_degeneracy_condK_frac', []).append(operator_degeneracy_condK_frac_val)
            if operator_degeneracy_detg_frac_val is not None:
                self.diagnostics.setdefault('operator_degeneracy_detg_frac', []).append(operator_degeneracy_detg_frac_val)
            if domain_applied_val is not None:
                self.diagnostics.setdefault('domain_applied', []).append(domain_applied_val)
            if domain_scale_val is not None:
                self.diagnostics.setdefault('domain_scale', []).append(domain_scale_val)
            if domain_pressure_val is not None:
                self.diagnostics.setdefault('domain_pressure', []).append(domain_pressure_val)
            if domain_mass_before_val is not None:
                self.diagnostics.setdefault('domain_mass_before', []).append(domain_mass_before_val)
            if domain_mass_after_val is not None:
                self.diagnostics.setdefault('domain_mass_after', []).append(domain_mass_after_val)
            if domain_mass_rel_error_val is not None:
                self.diagnostics.setdefault('domain_mass_rel_error', []).append(domain_mass_rel_error_val)
            if domain_mass_target_rel_error_val is not None:
                self.diagnostics.setdefault('domain_mass_target_rel_error', []).append(domain_mass_target_rel_error_val)
            if domain_I_mass_before_val is not None:
                self.diagnostics.setdefault('domain_I_mass_before', []).append(domain_I_mass_before_val)
            if domain_I_mass_after_val is not None:
                self.diagnostics.setdefault('domain_I_mass_after', []).append(domain_I_mass_after_val)
            if domain_I_mass_rel_error_val is not None:
                self.diagnostics.setdefault('domain_I_mass_rel_error', []).append(domain_I_mass_rel_error_val)
            ...
            np.savez(filename,
                     ...
                     nonlocal_term_mean=float(nonlocal_term_mean_val) if nonlocal_term_mean_val is not None else 0.0,
                     nonlocal_term_rms=float(nonlocal_term_rms_val) if nonlocal_term_rms_val is not None else 0.0,
                     operator_detK_mean=float(operator_detK_mean_val) if operator_detK_mean_val is not None else 0.0,
                     operator_detK_min=float(operator_detK_min_val) if operator_detK_min_val is not None else 0.0,
                     operator_condK_mean=float(operator_condK_mean_val) if operator_condK_mean_val is not None else 0.0,
                     operator_condK_max=float(operator_condK_max_val) if operator_condK_max_val is not None else 0.0,
                     operator_detg_mean=float(operator_detg_mean_val) if operator_detg_mean_val is not None else 0.0,
                     operator_detg_min=float(operator_detg_min_val) if operator_detg_min_val is not None else 0.0,
                     operator_g_drift_rms=float(operator_g_drift_rms_val) if operator_g_drift_rms_val is not None else 0.0,
                     operator_degeneracy_detK_frac=float(operator_degeneracy_detK_frac_val) if operator_degeneracy_detK_frac_val is not None else 0.0,
                     operator_degeneracy_condK_frac=float(operator_degeneracy_condK_frac_val) if operator_degeneracy_condK_frac_val is not None else 0.0,
                     operator_degeneracy_detg_frac=float(operator_degeneracy_detg_frac_val) if operator_degeneracy_detg_frac_val is not None else 0.0,
                     domain_applied=float(domain_applied_val) if domain_applied_val is not None else 0.0,
                     domain_scale=float(domain_scale_val) if domain_scale_val is not None else 1.0,
                     domain_pressure=float(domain_pressure_val) if domain_pressure_val is not None else 0.0,
                     domain_mass_before=float(domain_mass_before_val) if domain_mass_before_val is not None else 0.0,
                     domain_mass_after=float(domain_mass_after_val) if domain_mass_after_val is not None else 0.0,
                     domain_mass_rel_error=float(domain_mass_rel_error_val) if domain_mass_rel_error_val is not None else 0.0,
                     domain_mass_target_rel_error=float(domain_mass_target_rel_error_val) if domain_mass_target_rel_error_val is not None else 0.0,
                     domain_I_mass_before=float(domain_I_mass_before_val) if domain_I_mass_before_val is not None else 0.0,
                     domain_I_mass_after=float(domain_I_mass_after_val) if domain_I_mass_after_val is not None else 0.0,
                     domain_I_mass_rel_error=float(domain_I_mass_rel_error_val) if domain_I_mass_rel_error_val is not None else 0.0)
    ...
    def get_all(self):
        ...
        if self.storage_mode == 'memory':
            ...
            for f in files:
                ...
                diagnostics['nonlocal_term_mean'].append(float(data['nonlocal_term_mean']) if 'nonlocal_term_mean' in data else 0.0)
                diagnostics['nonlocal_term_rms'].append(float(data['nonlocal_term_rms']) if 'nonlocal_term_rms' in data else 0.0)
                diagnostics['operator_detK_mean'].append(float(data['operator_detK_mean']) if 'operator_detK_mean' in data else 0.0)
                diagnostics['operator_detK_min'].append(float(data['operator_detK_min']) if 'operator_detK_min' in data else 0.0)
                diagnostics['operator_condK_mean'].append(float(data['operator_condK_mean']) if 'operator_condK_mean' in data else 0.0)
                diagnostics['operator_condK_max'].append(float(data['operator_condK_max']) if 'operator_condK_max' in data else 0.0)
                diagnostics['operator_detg_mean'].append(float(data['operator_detg_mean']) if 'operator_detg_mean' in data else 0.0)
                diagnostics['operator_detg_min'].append(float(data['operator_detg_min']) if 'operator_detg_min' in data else 0.0)
                diagnostics['operator_g_drift_rms'].append(float(data['operator_g_drift_rms']) if 'operator_g_drift_rms' in data else 0.0)
                diagnostics['operator_degeneracy_detK_frac'].append(float(data['operator_degeneracy_detK_frac']) if 'operator_degeneracy_detK_frac' in data else 0.0)
                diagnostics['operator_degeneracy_condK_frac'].append(float(data['operator_degeneracy_condK_frac']) if 'operator_degeneracy_condK_frac' in data else 0.0)
                diagnostics['operator_degeneracy_detg_frac'].append(float(data['operator_degeneracy_detg_frac']) if 'operator_degeneracy_detg_frac' in data else 0.0)
                diagnostics['domain_applied'].append(float(data['domain_applied']) if 'domain_applied' in data else 0.0)
                diagnostics['domain_scale'].append(float(data['domain_scale']) if 'domain_scale' in data else 1.0)
                diagnostics['domain_pressure'].append(float(data['domain_pressure']) if 'domain_pressure' in data else 0.0)
                diagnostics['domain_mass_before'].append(float(data['domain_mass_before']) if 'domain_mass_before' in data else 0.0)
                diagnostics['domain_mass_after'].append(float(data['domain_mass_after']) if 'domain_mass_after' in data else 0.0)
                diagnostics['domain_mass_rel_error'].append(float(data['domain_mass_rel_error']) if 'domain_mass_rel_error' in data else 0.0)
                diagnostics['domain_mass_target_rel_error'].append(float(data['domain_mass_target_rel_error']) if 'domain_mass_target_rel_error' in data else 0.0)
                diagnostics['domain_I_mass_before'].append(float(data['domain_I_mass_before']) if 'domain_I_mass_before' in data else 0.0)
                diagnostics['domain_I_mass_after'].append(float(data['domain_I_mass_after']) if 'domain_I_mass_after' in data else 0.0)
                diagnostics['domain_I_mass_rel_error'].append(float(data['domain_I_mass_rel_error']) if 'domain_I_mass_rel_error' in data else 0.0)
...
if not HEADLESS_MODE:
    if USE_VISPY:
        viewer = RCVispyViewer(Nx, Ny, C_min, C_max, title="RC-PDE v16 CUDA")
    else:
        fig = plt.figure(figsize=(13,7))
        gs = fig.add_gridspec(2, 3, height_ratios=[6,1])
...
last_operator_state = _zero_operator_state()
last_domain_state = _zero_domain_state()

# ============================================================
# SIMULATION STEP
# ============================================================
def step_events(C, g_xx, g_xy, g_yy, step):
    """Event phase routing for v16 modes (L1 diagnostics + optional control signal)."""
    ...
    return spark_mask, event_state, spark_soft


def step_core(C, g_xx, g_xy, g_yy, target_mass, spark_mask, I_sum):
    """Reflexive core phase."""
    return rk2_step(C, g_xx, g_xy, g_yy, target_mass, spark_mask, I_sum)


def step_domain(C, g_xx, g_xy, g_yy, I_cont, I_tensor, n_active, target_mass, step):
    """Adaptive-domain proxy with conservative remap and mass diagnostics."""
    if (
        domain_mode != "adaptive"
        or domain_adapt_strength <= 0.0
        or (step % domain_adapt_interval != 0)
    ):
        return C, g_xx, g_xy, g_yy, I_cont, I_tensor, n_active, _zero_domain_state()

    _, sqrt_g_before, *_ = metric_det_and_inv(g_xx, g_xy, g_yy)
    mass_before_t = torch.sum(C * sqrt_g_before) * dx * dy

    structured = torch.clamp(C - coherence_bg_floor, min=0.0)
    structured_density = torch.mean(structured)
    dC_dx, dC_dy = compute_gradients(C)
    grad_rms = torch.sqrt(torch.mean(dC_dx * dC_dx + dC_dy * dC_dy) + eps)
    pressure_t = 0.5 * torch.clamp(structured_density / 0.25, 0.0, 1.0) + 0.5 * torch.clamp(grad_rms / 1.0, 0.0, 1.0)

    scale_delta = DOMAIN_SCALE_GAIN * domain_adapt_strength * pressure_t
    scale_t = torch.clamp(1.0 + scale_delta, min=1.0, max=1.0 + DOMAIN_SCALE_GAIN * domain_adapt_strength)

    C = remap_field_bilinear(C, scale_t)
    g_xx = remap_field_bilinear(g_xx, scale_t)
    g_xy = remap_field_bilinear(g_xy, scale_t)
    g_yy = remap_field_bilinear(g_yy, scale_t)
    g_xx, g_xy, g_yy, _ = regularise_metric(g_xx, g_xy, g_yy)

    _, sqrt_g_after, *_ = metric_det_and_inv(g_xx, g_xy, g_yy)
    I_mass_before_t = torch.sum(I_cont * sqrt_g_before) * dx * dy
    I_mass_after_t = I_mass_before_t

    I_cont = remap_field_bilinear(I_cont, scale_t)
    I_cont_mass_after = torch.sum(I_cont * sqrt_g_after) * dx * dy
    I_cont_scale = torch.clamp(I_mass_before_t / (I_cont_mass_after + eps), min=0.0, max=10.0)
    I_cont *= I_cont_scale
    I_mass_after_t = torch.sum(I_cont * sqrt_g_after) * dx * dy

    I_discrete_before_t = torch.tensor(0.0, device=device)
    I_discrete_after_t = torch.tensor(0.0, device=device)
    if n_active > 0:
        I_active = remap_field_bilinear(I_tensor[:n_active], scale_t)
        I_discrete_before_t = torch.sum(I_tensor[:n_active] * sqrt_g_before.unsqueeze(0)) * dx * dy
        I_discrete_after_t = torch.sum(I_active * sqrt_g_after.unsqueeze(0)) * dx * dy
        I_scale = torch.clamp(I_discrete_before_t / (I_discrete_after_t + eps), min=0.0, max=10.0)
        I_active *= I_scale
        I_tensor[:n_active] = I_active
        I_discrete_after_t = torch.sum(I_tensor[:n_active] * sqrt_g_after.unsqueeze(0)) * dx * dy

    C = project_to_invariant(C, g_xx, g_xy, g_yy, target_mass)
    mass_after_t = torch.sum(C * sqrt_g_after) * dx * dy
    I_mass_before_t = I_mass_before_t + I_discrete_before_t
    I_mass_after_t = I_mass_after_t + I_discrete_after_t
    mass_rel_error_t = (mass_after_t - mass_before_t) / (torch.abs(mass_before_t) + eps)
    mass_target_rel_error_t = (mass_after_t - target_mass) / (abs(target_mass) + eps)
    I_mass_rel_error_t = (I_mass_after_t - I_mass_before_t) / (torch.abs(I_mass_before_t) + eps)

    domain_state = {
        "applied_t": torch.tensor(1.0, device=device),
        "scale_t": scale_t,
        "pressure_t": pressure_t,
        "mass_before_t": mass_before_t,
        "mass_after_t": mass_after_t,
        "mass_rel_error_t": mass_rel_error_t,
        "mass_target_rel_error_t": mass_target_rel_error_t,
        "I_mass_before_t": I_mass_before_t,
        "I_mass_after_t": I_mass_after_t,
        "I_mass_rel_error_t": I_mass_rel_error_t,
    }
    return C, g_xx, g_xy, g_yy, I_cont, I_tensor, n_active, domain_state


def step_closure(C, I_tensor, n_active, spark_mask, spark_soft, sqrt_g, dt, step, I_cont_mass_t):
    """Closure phase routing for v16 modes."""
    if closure_mode == "off":
        # Keep closure side effects disabled; expose readout-only observables.
        if n_active > 0:
            I_tensor[:n_active].zero_()
        closure_state = compute_closure_observables(I_cont_mass_t, spark_soft, step)
...
def update(frame):
    global C, g_xx, g_xy, g_yy, I_cont, I_tensor, n_active, I_sum, step_counter
    global last_dt, last_mass, last_I_mass, last_collapse_score_mean, last_collapse_score_max
    global last_event_state, last_closure_state, last_operator_state, last_domain_state
    ...
    spark_mask, event_state, spark_soft = step_events(C, g_xx, g_xy, g_yy, step_counter)
    if I_sum is None:
        core_I_sum = I_cont
    else:
        core_I_sum = I_cont + I_sum

    C, g_xx, g_xy, g_yy, dt, vu, vv, operator_state = step_core(
        C, g_xx, g_xy, g_yy, target_mass0_value, spark_mask, core_I_sum
    )
    C, g_xx, g_xy, g_yy, I_cont, I_tensor, n_active, domain_state = step_domain(
        C, g_xx, g_xy, g_yy, I_cont, I_tensor, n_active, target_mass0_value, step_counter
    )
    _hook("post_core_pre_closure", dt=dt, C_field=C)

    det_g, sqrt_g, *_ = metric_det_and_inv(g_xx, g_xy, g_yy)
    I_cont, I_cont_mass_t = update_continuous_identity(I_cont, C, spark_soft, sqrt_g, dt)
    I_tensor, n_active, I_sum, closure_state = step_closure(
        C, I_tensor, n_active, spark_mask, spark_soft, sqrt_g, dt, step_counter, I_cont_mass_t
    )
    ...
    if closure_mode == "off":
        collapse_score_mean_t, collapse_score_max_t = compute_continuous_collapse_scores(I_cont)
    else:
        collapse_score_mean_t, collapse_score_max_t = compute_intrinsic_collapse_scores(I_tensor, n_active, sqrt_g)

    # Identity diagnostics
    if I_sum is not None:
        I_mass = torch.sum((I_cont + I_sum) * sqrt_g) * dx * dy
    else:
        I_mass = torch.sum(I_cont * sqrt_g) * dx * dy
    ...
    if operator_diagnostics and (step_counter % operator_diag_interval == 0):
        last_operator_state = operator_state
    elif not operator_diagnostics:
        last_operator_state = _zero_operator_state()
    ...
    if HEADLESS_MODE and snapshot_frame:
        nonlocal_mean_val, nonlocal_rms_val = compute_nonlocal_diagnostics(C)
        operator_detK_mean_val = float(last_operator_state["detK_mean_t"].item())
        operator_detK_min_val = float(last_operator_state["detK_min_t"].item())
        operator_condK_mean_val = float(last_operator_state["condK_mean_t"].item())
        operator_condK_max_val = float(last_operator_state["condK_max_t"].item())
        operator_detg_mean_val = float(last_operator_state["detg_mean_t"].item())
        operator_detg_min_val = float(last_operator_state["detg_min_t"].item())
        operator_g_drift_rms_val = float(last_operator_state["g_drift_rms_t"].item())
        operator_degeneracy_detK_frac_val = float(last_operator_state["degeneracy_detK_frac_t"].item())
        operator_degeneracy_condK_frac_val = float(last_operator_state["degeneracy_condK_frac_t"].item())
        operator_degeneracy_detg_frac_val = float(last_operator_state["degeneracy_detg_frac_t"].item())
        domain_applied_val = float(last_domain_state["applied_t"].item())
        domain_scale_val = float(last_domain_state["scale_t"].item())
        domain_pressure_val = float(last_domain_state["pressure_t"].item())
        domain_mass_before_val = float(last_domain_state["mass_before_t"].item())
        domain_mass_after_val = float(last_domain_state["mass_after_t"].item())
        domain_mass_rel_error_val = float(last_domain_state["mass_rel_error_t"].item())
        domain_mass_target_rel_error_val = float(last_domain_state["mass_target_rel_error_t"].item())
        domain_I_mass_before_val = float(last_domain_state["I_mass_before_t"].item())
        domain_I_mass_after_val = float(last_domain_state["I_mass_after_t"].item())
        domain_I_mass_rel_error_val = float(last_domain_state["I_mass_rel_error_t"].item())
        snap_mgr.add_snapshot(
            ...
            nonlocal_term_mean_val=nonlocal_mean_val,
            nonlocal_term_rms_val=nonlocal_rms_val,
            operator_detK_mean_val=operator_detK_mean_val,
            operator_detK_min_val=operator_detK_min_val,
            operator_condK_mean_val=operator_condK_mean_val,
            operator_condK_max_val=operator_condK_max_val,
            operator_detg_mean_val=operator_detg_mean_val,
            operator_detg_min_val=operator_detg_min_val,
            operator_g_drift_rms_val=operator_g_drift_rms_val,
            operator_degeneracy_detK_frac_val=operator_degeneracy_detK_frac_val,
            operator_degeneracy_condK_frac_val=operator_degeneracy_condK_frac_val,
            operator_degeneracy_detg_frac_val=operator_degeneracy_detg_frac_val,
            domain_applied_val=domain_applied_val,
            domain_scale_val=domain_scale_val,
            domain_pressure_val=domain_pressure_val,
            domain_mass_before_val=domain_mass_before_val,
            domain_mass_after_val=domain_mass_after_val,
            domain_mass_rel_error_val=domain_mass_rel_error_val,
            domain_mass_target_rel_error_val=domain_mass_target_rel_error_val,
            domain_I_mass_before_val=domain_I_mass_before_val,
            domain_I_mass_after_val=domain_I_mass_after_val,
            domain_I_mass_rel_error_val=domain_I_mass_rel_error_val,
        )
...
if not HEADLESS_MODE:
    ...
    if snap_mgr.has_data() is False:
        initial_spark_mask, initial_event_state, _ = step_events(C, g_xx, g_xy, g_yy, step=0)
        initial_mass = torch.sum(C * sqrt_g0) * dx * dy
        initial_I_mass = float((torch.sum(I_cont * sqrt_g0) * dx * dy).item())
        initial_collapse_score_mean_t, initial_collapse_score_max_t = compute_intrinsic_collapse_scores(I_tensor, n_active, sqrt_g0)
        initial_nonlocal_mean, initial_nonlocal_rms = compute_nonlocal_diagnostics(C)
        zero_closure = 0.0
        zero_operator = 0.0
        zero_domain = 0.0
        snap_mgr.add_snapshot(
            ...
            nonlocal_term_mean_val=initial_nonlocal_mean,
            nonlocal_term_rms_val=initial_nonlocal_rms,
            operator_detK_mean_val=zero_operator,
            operator_detK_min_val=zero_operator,
            operator_condK_mean_val=zero_operator,
            operator_condK_max_val=zero_operator,
            operator_detg_mean_val=zero_operator,
            operator_detg_min_val=zero_operator,
            operator_g_drift_rms_val=zero_operator,
            operator_degeneracy_detK_frac_val=zero_operator,
            operator_degeneracy_condK_frac_val=zero_operator,
            operator_degeneracy_detg_frac_val=zero_operator,
            domain_applied_val=zero_domain,
            domain_scale_val=1.0,
            domain_pressure_val=zero_domain,
            domain_mass_before_val=float(initial_mass.item()),
            domain_mass_after_val=float(initial_mass.item()),
            domain_mass_rel_error_val=zero_domain,
            domain_mass_target_rel_error_val=zero_domain,
            domain_I_mass_before_val=zero_domain,
            domain_I_mass_after_val=zero_domain,
            domain_I_mass_rel_error_val=zero_domain,
        )
    ...
    if step_counter > 0 and (not snap_mgr.steps or snap_mgr.steps[-1] != step_counter):
        ...
        final_nonlocal_mean, final_nonlocal_rms = compute_nonlocal_diagnostics(C)
        snap_mgr.add_snapshot(
            step=step_counter,
            ...
            nonlocal_term_mean_val=final_nonlocal_mean,
            nonlocal_term_rms_val=final_nonlocal_rms,
            operator_detK_mean_val=float(last_operator_state["detK_mean_t"].item()),
            operator_detK_min_val=float(last_operator_state["detK_min_t"].item()),
            operator_condK_mean_val=float(last_operator_state["condK_mean_t"].item()),
            operator_condK_max_val=float(last_operator_state["condK_max_t"].item()),
    ...
    if dt_hist:
        ...
        print(
            f"[EVENT] spark_score_mean={float(last_event_state['spark_score_mean_t'].item()):.6e}  "
            f"spark_score_max={float(last_event_state['spark_score_max_t'].item()):.6e}  "
            f"collapse_score_mean={last_collapse_score_mean:.6e}  "
            f"collapse_score_max={last_collapse_score_max:.6e}  "
            f"closure_birth_score={float(last_closure_state['birth_score_t'].item()):.6e}  "
            f"closure_sparks_factor={float(last_closure_state['sparks_factor_t'].item()):.6e}  "
            f"closure_mass_factor={float(last_closure_state['mass_factor_t'].item()):.6e}  "
            f"closure_interval_factor={float(last_closure_state['interval_factor_t'].item()):.6e}  "
            f"closure_births={float(last_closure_state['n_new_t'].item()):.6e}"
        )
        print(
            f"[NONLOCAL] mode={nonlocal_mode}  strength={nonlocal_strength:.3f}  "
            f"term_mean={final_nonlocal_mean:.6e}  term_rms={final_nonlocal_rms:.6e}"
        )
        if operator_diagnostics:
            print(
                f"[OPERATOR] detK_mean={float(last_operator_state['detK_mean_t'].item()):.6e}  "
                f"detK_min={float(last_operator_state['detK_min_t'].item()):.6e}  "
                f"condK_mean={float(last_operator_state['condK_mean_t'].item()):.6e}  "
                f"condK_max={float(last_operator_state['condK_max_t'].item()):.6e}  "
                f"detg_mean={float(last_operator_state['detg_mean_t'].item()):.6e}  "
                f"detg_min={float(last_operator_state['detg_min_t'].item()):.6e}  "
                f"g_drift_rms={float(last_operator_state['g_drift_rms_t'].item()):.6e}  "
                f"deg_detK_frac={float(last_operator_state['degeneracy_detK_frac_t'].item()):.6e}  "
                f"deg_condK_frac={float(last_operator_state['degeneracy_condK_frac_t'].item()):.6e}  "
                f"deg_detg_frac={float(last_operator_state['degeneracy_detg_frac_t'].item()):.6e}"
            )
        if domain_mode == "adaptive" and domain_adapt_strength > 0.0:
            print(
                f"[DOMAIN] applied={float(last_domain_state['applied_t'].item()):.0f}  "
                f"scale={float(last_domain_state['scale_t'].item()):.6f}  "
                f"pressure={float(last_domain_state['pressure_t'].item()):.6e}  "
                f"mass_rel_error={float(last_domain_state['mass_rel_error_t'].item()):.6e}  "
                f"mass_target_rel_error={float(last_domain_state['mass_target_rel_error_t'].item()):.6e}  "
                f"I_mass_rel_error={float(last_domain_state['I_mass_rel_error_t'].item()):.6e}"
            )
