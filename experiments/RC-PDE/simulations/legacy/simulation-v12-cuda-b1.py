...
print(f"Initial coherence mass: {target_mass0.item():.4f}")
...
def update(frame):
    ...
    C, g_xx, g_xy, g_yy, dt, vu, vv = rk2_step(
        C, g_xx, g_xy, g_yy, target_mass0.item(), spark_mask, I_sum
    )
    ...
    # Update diagnostics only on sampled frames (throttle host sync)
    if sampled_frame:
        dt_hist.append(dt)
