...
The document started as a compressed summary of an investigation about the practical meaning of ROM and how to apply it in everyday life. Although the initial direction of the exploration was more about the meaning and interpretation of purpose functional and reflexive conservation principle (RCP), the idea diverged, exploring what makes some structures and currents more "likeable" than others. This led to the idea of a formal definition of a "seed". It ends up with hypothesis how this idea alters the underlying properties of processes like natural selection. The document summaries the emergence of the formalisms.
...
We'll start with something quite general, the reserves as one of the attributes that determine the level of change. Then we'll look at why some activity feels more natural than others. We'll define seeds and explore the potential of the organism. In the end, we'll explore how to move away from abstract mathematical formulations and rather start thinking in terms of biological computation.
...
Reserves matter when considering the effect of activity in one step, execution of one reflexive loop. They are not needed from the conceptual view of ROM itself, they are the equations for $\mathcal{J}$, RPC and operators define everything needed to set the boundaries. However, when we start thinking about application in terms of quantity, we need to know the values that the equations define.
...
The continuity equation simply states that *any increase of compatible information at a point is either because it flows in from elsewhere, is created on‑site, or survives the local loss processes*.
...
$$  
\epsilon_{reuse} = \frac{\langle \mathcal J , \mathcal J_{struct}\rangle} {\|\mathcal J\|\ \|\mathcal J_{struct}\|},
$$
...
$$  
I_{beat} = \int_V |\mathcal J|dV \approx \epsilon_{reuse}\ s_{parent}\ V_{eff},  
$$
...
The idea of the spark is based on a topic already covered in the model when discussing rotation. The heart of ROM is the reflexive cycle
...
$$
\underbrace{\text{Perception}}_{P(t)} \xrightarrow{j_i^\mu(t)} \underbrace{\text{Memory update}}_{M(t+\Delta t)} \xrightarrow{\text{Write‑back}}\ \underbrace{\text{Action}}_{A(t+\Delta t)} \xrightarrow{\text{Feedback}}\ P(t+\Delta t) .
$$
...
Besides reserves and the embedded structural motifs of the seeds, asymmetry can arise from dynamic $\lambda$‑mixing between altruistic (cooperative) and selfish (exploratory) behaviors. The mixing coefficient $\lambda(t)\in[0,1]$ effectively scales the **gain contributed by exploratory actions**
...
$$
\mathcal{E}_{\uparrow}:\mathcal J^{(\text{child})}\rightarrow \mathcal J^{(\text{super})}, \qquad \mathcal{E}_{\downarrow}:\rho_{\text{compat}}^{(\text{super})}\rightarrow\rho_{\text{compat}}^{(\text{child})}.
$$
...
$$
\mathcal{J}^{(\text{super})} = \underbrace{\sum_{Q} w_QP_Q}_{\displaystyle\mathcal{E}_{\uparrow}}\mathcal{J}^{(Q)} .
$$
...
$$
\rho_{\text{compat}}^{(Q)} = \underbrace{R_Q}_{\displaystyle\mathcal{E}_{\downarrow}} \rho_{\text{compat}}^{(\text{super})}.
$$
...
The first gives us *where* coherence can most easily arise, the second records *how* it actually stabilized during life. Because the reflexive field and the genome co-evolve, information flows both ways over evolutionary time. In short term, the field reads the genome shaping its developmental expression. On the long term, stable field configurations (behavioral niches, morphodynamic constraints) feed back into selective pressures, slowly **writing curvature back** into DNA. So the genome is the **slow-changing memory** of successful reflexive structures.

In this view:

- DNA encodes **curvature**, not explicit behavior.
...
Now, let's make a high-level overview how this "DNA-as-curvature" hypothesis actually lines up with what's empirically known.

- **DNA as slow manifold / constraint set**. The genome does not specify every molecular state. It encodes rules for how local chemical and regulatory fields will interact. Developmental biology shows that gene regulatory networks, morphogenic gradients, and mechanical feedback loops together generate form through *self-organization*, not blueprinting.  The genome defines what reaction–diffusion couplings are possible.
...
$$
\int_{\Gamma_{\text{interface}}} \mathbf{n}\cdot\mathcal J^{(child)} = - \int_{\Gamma_{\text{interface}}} \mathbf{n}\cdot\mathcal J^{(super)} .
$$

That expresses the intuitive fact that the child both draws coherence from and contributes balance to the parent. In biological terms this manifests as ecological roles or cultural functions and in energetic terms as resource and information exchange.
...
$$
\nabla_i^{(g)}(\cdot)=g^{ij}\partial_j(\cdot), \qquad \Delta_g(\cdot) = \nabla_i^{(g)}\nabla^i_{(g)}(\cdot).
$$
...
**Slow memory** (the “seed’’ itself). The seed variables are not static. They evolve on the longest timescale through accumulated coherency budget:
...
$$
-\Delta_g \phi_k = \lambda_k \phi_k ,\qquad L_{\mathcal G}\phi_k = \mu_k\phi_k ,
$$

where $L_{\mathcal G}$ is the graph Laplacian that couples sub‑colonies. Expansions are as follows
...
By inserting these expansions into PDEs, multiplied by $\phi_m$ and integrated over $\Omega$, because the basis diagonalises the diffusion operator, we obtain a system of ODEs for the modal amplitudes

$$
\begin{aligned}
\dot a_m &= -\lambda_m\tilde D_m(\theta,g,\mathcal B)a_m - \lambda(\theta) a_m + \alpha_s b_m+\alpha_r c_m + \underbrace{\langle\mathcal B, \nabla\phi_m\rangle}_{\text{bias term}},\\
\dot b_m &= -\lambda_m D^{s}_m(\theta,g,\mathcal B)b_m + \chi_s\sum_{k} a_kQ_{km}^{(s)} + f_s^{(m)}(b,c,M), \\
\dot c_m &= -\lambda_m D^{r}_m(\theta,g,\mathcal B)c_m - \chi_r\sum_{k} a_kQ_{km}^{(r)} + f_r^{(m)}(b,c,F,M),
\end{aligned}
$$
...
$$
\begin{aligned}
\partial_t s &= -\nabla\cdot \mathbf{J}_s -\kappa_{sf} sF +\sigma_r r ,\\
\partial_t r &= -\nabla\cdot \mathbf{J}_r +\kappa_{sf} sF -\sigma_r r .
\end{aligned}
$$
...
$$
\partial_t p = D_p\Delta p +\alpha_s s+\alpha_r r -\lambda_p p ,
$$
...
\dot E &= \eta_f\kappa_{sf}\int_\Omega sFdx - \eta_c \int_\Omega (s+r)dx ,
\end{aligned}
$$
...
$$
\rho(\mathcal F)>1 \Longleftrightarrow \exists \mathbf{k} \text{ such that } \Re\bigl[\lambda_{\mathbf{k}}(J)\bigr] > 0 .
$$
...
Putting everything together, the colony will be **spark‑activated** when **all** the following hold:
...
$$
\int_{\Gamma_{\text{interface}}} \mathbf n\cdot\mathbf J_{\text{child}} = -\int_{\Gamma_{\text{interface}}} \mathbf n\cdot\mathbf J_{\text{parent}},  
$$
